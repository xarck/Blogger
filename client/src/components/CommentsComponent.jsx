...
import CommentCard from "./CommentCardComponent";
        ...
        <div className={"max-sm:w-full fixed " + (commentsWrapper ? "top-0 sm:right-0 " : " top-[100%] sm:right-[-100%] ") + " duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden "}>
            <div className="relative">
                ...
                <button
                    ...
                    className="absolute top-0 right-0 flex justify-center items-center w-10 h-10 rounded-full bg-grey transform transition-all duration-300 hover:scale-110  ">
                    <i className="fi fi-br-cross-circle text-2xl mt-1"></i>
                </button>
            </div>
...

            {
                commentsArr && commentsArr.length ?
                    ...
                    }) : <NoDataComponent message="No Comments" />
            }

            {
                ...
                total_parent_comments > totalParentCommentsLoaded ? <button
                    ...
                    className="text-dark-grey p-2 px-3  hover:bg-grey/60 rounded-md flex items-center gap-2">Load More</button> : ""
            }
        </div>
    )
}
