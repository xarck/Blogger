...
import NotificationCommentFeild from "../components/NotificationCommentComponent";
    ...
    return (
        ...
        <div className={"p-6 border-b border-grey border-l-black " + (!seen ? "border-l-2 " : "")} >
                ...
                <div className="w-full ">
                    <h1 className="font-medium text-xl text-dark-grey">
                        ...
                        <span className=" font-normal">
                            {
                                type == 'like' ? "liked your blog ❤️" :
                                    type == 'comment' ? "commented on blog" : "replied on "
                            }
                        </span>
                    </h1>
                    ...
                    type != 'like' ?
                        <>
                            ...
                            <button className="underline hover:text-black "
                                onClick={(e) => handleDelete(comment._id, "comment", e.target)}>Delete
                            </button>
                        </>
                        : ""
                }
...

            {
                isReplying ?
                    <div className="mt-8 ">
                        <NotificationCommentFeild _id={_id} blog_author={user} index={index} replyingTo={comment._id} setIsReplying={setIsReplying} notification_id={notification_id} notificationData={notificationState} />
                    </div>
                    : ""
            }
                            ...
                            <div>
                                <h1 className="font-medium text-xl text-dark-grey ">
                                    <Link to={`/user/${author_username}`}
                                        className="mx-1 text-black underline">@{author_username}
                                    </Link>

                                    <span className="font-normal ">replied to</span>
                                    <Link to={`/user/${username}`}
                                        className="mx-1 text-black underline">@{username}
                                    </Link>
                                </h1>
                            </div>
                        </div>
