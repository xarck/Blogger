        ...
        .then(({ data }) => {
            ...
            setStateFun(preVal => {
                ...
                if (!results.length && totalDocs - 1 > 0) {
                    return null; //will setstate to null which triggers the useEffect to  load next blog without clicking on load more button 
                }
    ...
    return (
        <>
            <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                <img src={banner} className="max-md:hidden lg:hidden xl:block w-28 h-28 flex-none bg-grey object-cover rounded-sm" />

                <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
                    <div>
                        <Link to={`/blog/${blog_id}`} className="blog-title mb-4 hover:scale-105  transition duration-300 ease-in-out ">{title}</Link>
...
export const ManageDraftBlogPost = ({ blog }) => {
    ...
    index++; //incrementing index of arr
