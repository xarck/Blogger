...
import handleImageUpload from "../common/Cloudinary"
    ...
    const handleUploadImage = async (e) => {
        ...
        if (updatedProfileImg) {
            try {
                ...
                const imageUrl = await handleImageUpload(updatedProfileImg);

                // Set the uploaded image URL in your component state or use it as needed
                console.log('Uploaded Image URL:', imageUrl);
                if (imageUrl) {
                    axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/update-profile-img", { url: imageUrl }, {
                        headers: {
                    ...
                    <form ref={editProfileForm}>
                        ...
                        <div className="flex flex-col lg:flex-row items-start py-10 gap-8 lg:gap-10">
                            ...
                            <div className="max-lg:center mb-5 ">
                                <label htmlFor="uploadImg" id="profileImgLable" className="relative block w-48 h-48 bg-grey rounded-full overflow-hidden ">
                                    <div className=" w-full h-full absolute top-0 left-0 flex items-center justify-center text-white bg-black/40 opacity-0 hover:opacity-100 cursor-pointer  ">
                                        <i className="fi fi-rr-add text-4xl flex items-center justify-center"></i>
                                    </div>
                                ...
                                <button className="btn-light mt-5 max-lg:center lg:w-full px-10" onClick={handleUploadImage}> Upload Image</button>
                            </div>
                            ...
                            <div className="w-full ">
                                ...
                                <textarea name="bio" maxLength={bioLimit} defaultValue={bio} className="input-box h-64 lg:h-40 resize-none leading-7 mt-5 pl-5" placeholder="Bio" onChange={hadleCharachterChange}></textarea>
                                <p className="mt-1 text-dark-grey  " >{charactersLeft} Characters Left </p>

                                <p className="my-6 text-dark-grey ">Social handels <i className="fi fi-sr-following text-xl"></i></p>
